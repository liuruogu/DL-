...
object Main {
  def main(args: Array[String]) {
    ...
    val Array(trainingData, testData) = obsDF.randomSplit(Array(0.7,0.3),199)
    ...
    val fSlicer = new VectorSlicer()
      .setInputCol("vectors")
      .setOutputCol("singlefeatures")

    fSlicer.setIndices(Array(1,2,3))
    
    //create the polynomial expansion transformer
    val polynomialExpansionT = new PolynomialExpansion().setInputCol("singlefeatures").setOutputCol("features").setDegree(2)
    ...
    //Build the parameter grid to choose by setting the values for maxIter and regParam
